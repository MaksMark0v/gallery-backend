...
export async function getUsersData(query) { 
    ...
    const users = data.result.map((user) => { // Створення нового масиву користувачів з потрібними полями
        return { 
    ...
    if (query.page && query.size) { // Перевірка наявності параметрів пагінації у запиті
    ...
    if (query['filter[Status]'] === 'active') { // Перевірка наявності фільтру 'active' у запиті
        paginatedUsers = paginatedUsers.filter((user) => user.Status === 'active'); // Вибірка активних користувачів        
    }
